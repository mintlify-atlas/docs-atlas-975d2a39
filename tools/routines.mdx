...
description: "All 9 Paperclip routine MCP tools: create routines, add cron and event triggers, run manually, and inspect execution history. Automate recurring agent workflows."
---
...
<ParamField body="concurrencyPolicy" type="string">
  Controls what happens when a new run fires while a previous one is still active. Values: `allow`, `forbid` (default), `replace`.
</ParamField>

<ParamField body="catchUpPolicy" type="string">
  Controls missed runs after downtime. Values: `skip` (default), `run_once`.
</ParamField>
